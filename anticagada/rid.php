<html>
	...
	<body>
		...
		//	if(0)
			...
			// EXIBIR RID
		//	if(0)
			echo"<div id='quadro'><table><tr><td id='titulo'>ACEITAS</td><td id='titulo'>NÃO ACEITAS</td></tr>";
			
			$bunda=$NumDisciplina;
			$DisciplinasRecusadasCont=$DisciplinasRecusadas;
			for(;$bunda>0;$bunda--){
				if($DisciplinasRecusadas) {$i=$Disciplinas[$bunda]->nome; $DisciplinasRecusadas--;}
				else {$i="";}
				if($DisciplinasAceitas) {$j=$Disciplinas[$bunda-$DisciplinasRecusadasCont]->nome; $DisciplinasAceitas--;}
				else {$j="";}
				echo"<tr><td id='dadosGrandes'>$j</td><td id='dadosGrandes'>$i</td></tr>";
				if (($DisciplinasAceitas==0)&&($DisciplinasRecusadas==0)) break; 
			}
			echo"</table></div>";
			...
			for($j=1;$j<=6;$j++){
				...
				for($i=1;$i<=8;$i++){
					...
					if ($Horario[$i][$j]["status"])
						if(($i>1)&&($i<8)) echo"<td><div id='dados'>".substr($Horario[$i][$j]["nome"],12)."</div></td>";
						else echo" ";
					else
