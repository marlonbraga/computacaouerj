<html>
	...
	<?php
		...
		//	if($tam)	echo "<br>$tam | [$i] => <b>$str[$i]</b>"; //  24 | [114] => 2011/1	
			...
			if($str[$i]=="Curso:") 			$curso=trim($str[$i+1]);
		...
		/*	if($str[$i]=="Grupo de Eletivas: Restritas Básicas"){
			...
			$anoAtual=20141;
                        $CRcolorido=colorirNota($CR);
	echo"<div style='float:left; width:480px;'>";
                echo"<div class='quadrinho'>";
			...
			echo"<font class='subtitulo'>CR:</font><font class='dados'>$CRcolorido<br></font>";
		...
		$criptomatricula=md5($matricula);
                $CR=str_replace(",", ".", $CR);
		$sql="REPLACE INTO `aluno` (`id` ,`matricula` ,`nome` ,`cr` ,`ano` ,`periodo` ,`conclusao` ,`curso` ,`atualizacao`)
                        VALUES (NULL ,  '$criptomatricula',  NULL,  '$CR',  '$ano',  '$PeríodosUtilizados',  '$porcentagemConclusao',  '".utf8_decode($curso)."',  '$anoAtual');
                       ;";
                //$sql2="UPDATE  `caime526_UERJ`.`aluno` SET  `nome` = '$nome' WHERE `aluno`.`matricula` = $criptomatricula;";
